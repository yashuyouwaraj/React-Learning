...
const GestureBasedImageGallery = () => {
  ...
  return (
    <div className="relative w-72 h-72 overflow-hidden">
        <motion.img src={images[currentIndex]} key={currentIndex} alt={`Slide ${currentIndex}`} drag="y" dragConstraints={{top:0, bottom:0}} onDragEnd={handleSwipe} initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="w-full h-full rounded" />
