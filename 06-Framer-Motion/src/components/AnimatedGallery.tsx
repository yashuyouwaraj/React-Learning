...
const AnimatedGallery = () => {
  ...
  return (
    <div>
        <button onClick={handleClick} className="mb-[2rem] p-4 rounded-lg bg-yellow-300 text-black font-bold">{showImages ? 'Hide Images':'Show Images'}</button>
        <motion.div variants={parentVariants} className="flex" initial='hidden' animate={showImages ? "visible":"hidden"}>
            {galleryImages.map((image,index)=>(
                <motion.img key={index} src={image} alt={`Gallery Image ${index}`} variants={childVariants} className="ml-[2rem] w-[300px] rounded" />
            ))}
        </motion.div>
    </div>
  )
